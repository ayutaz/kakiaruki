...
M6の受入条件（§6）のうち、**自動試験で検証できる7項目に合格**しました。描く → 学習 → 観察 → 描き直しを、開発ページへ触れずに `index.html` だけで完走できます。
...
**この確認が終わるまでM6は完了ではありません。** 技術試験がすべて通っていても、分かりにくければ完了にしません（[docs/13](13-milestone-quality-and-decision-gates.md) §6）。
