...
1. 100世代と30分連続runで、未処理例外・NaN・資源増加がないことを確かめる。
2. ChromiumとFirefoxで、描画からリプレイまでの主要flowを完走させる。
3. M2で確定したPopulation 32の性能基準を、製品画面で満たすか測る。
4. `npm audit` と非推奨の推移依存を再確認する。
5. 名称・UI・アセット・クレジットを人が確認する。
6. release candidateのhashを記録し、公開するかを人が決める。

M6では製品画面（`index.html`）を作り、開発ページへ触れずに描く → 学習 → 観察 → 描き直しを完走できるようにしました。**browser E2Eは未導入のままです**（§10 受入条件の1項目が未達）。

### M7と並行して片付けたいもの

実装の前後は問いませんが、次はM7の進行と独立して人が実施できます。
