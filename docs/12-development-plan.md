...
| M5 | 枝分かれと編集 | 一筆らしい複雑形状を扱う | Y字・人型相当・Undoが仕様どおり動く | P4 | 未着手 |
| M6 | 体験統合 | 個別機能を遊べる流れへまとめる | 描画からベスト個体のリプレイまで完走する | P5 | 未着手 |
| M7 | 安定化と公開判断 | MVP候補の品質と公開条件を確定する | 長時間・ブラウザ・権利確認を終え、公開可否を判断できる | P6 | 未着手 |
...
1. 戻り線（既存Edgeを逆向きになぞる）の検出仕様をfixtureとして固定する。
2. 既存Node／Edgeへのsnap規則の失敗テストを作る。
3. 次数3以上のNodeを含むGraphの生成と、M1の `buildSkeletonPlan` が作る（次数−1）個のJointとの対応を確認する。
4. Edge単位Undoと全消去を実装する。
5. Y字・人型相当・意図しない近接・交差・単純輪のfixtureを揃える。
6. 枝分かれGraphでM3の進化loopが完走することを確認する。
7. M5検証結果をdocsへ追加し、M6へ進めるか判定する。

M4では戻り線・自己交差・閉ループを**理由付きで拒否**しています。M5ではこの拒否を、どこまで受け入れる仕様へ変えるかを決めます。

### M5より先に片付けたいもの

実装の前後は問いませんが、次はM5の進行と独立して人が実施できます。
...
4. browser E2Eフレームワークを導入するかの判断（[docs/17](17-m4-stroke-input-validation.md) §8）。
