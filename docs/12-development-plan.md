...
1. 画面の状態遷移（描く → 準備完了 → 学習中 → 観察・リプレイ）を純粋な状態機械として固定する。
2. 学習中にGraphを書き換えられないことを試験で縛る。
3. 速度倍率がepisodeとfitnessの定義を変えないことを試験で縛る。
4. 停止・再開・reset・描き直しが資源を残さないことを確認する。
5. 製品画面（`index.html`）を組み、開発ページなしで一続きに操作できるようにする。
6. M6検証記録をdocsへ追加し、M7へ進めるか判定する。

M5では戻り線による枝分かれとEdge単位Undoまでを実装し、**閉ループと自己交差は理由付きで拒否したまま**です（§9 判断ゲートの規定どおり）。

### M6と並行して片付けたいもの

実装の前後は問いませんが、次はM6の進行と独立して人が実施できます。
