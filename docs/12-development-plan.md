...
この文書を、P0完了後の実装順序とマイルストーン完了判定の基準とします。[PoCロードマップ](07-poc-roadmap.md) は技術Phaseの全体像、この文書は現在地点からの具体的な実行計画です。
...
| Milestone | 名称 | 目的 | 完了時のゴール | 対応Phase |
|---|---|---|---|---|
| M1 | Simulation基盤 | 固定骨格を安定して評価する | 4〜6ボーンの1個体が繰り返しepisodeを完走する | P1前半 |
| M2 | Population評価と性能 | 複数個体を公平かつ高速に評価する | 1 WorldでPopulation 32を測定可能にする | P1後半 |
| M3 | 進化loop | Controllerが移動を学習できるか判断する | 複数Seedで進化群が対照群を上回る | P2 |
| M4 | 単純な一筆入力 | 描線を安全な物理Graphへ変換する | 直線・L字・ジグザグから学習を開始できる | P3 |
| M5 | 枝分かれと編集 | 一筆らしい複雑形状を扱う | Y字・人型相当・Undoが仕様どおり動く | P4 |
| M6 | 体験統合 | 個別機能を遊べる流れへまとめる | 描画からベスト個体のリプレイまで完走する | P5 |
| M7 | 安定化と公開判断 | MVP候補の品質と公開条件を確定する | 長時間・ブラウザ・権利確認を終え、公開可否を判断できる | P6 |
...
1. `CreatureGraph` とvalidationの仕様をtest fixtureとして固定する。
2. GraphからBody／Jointへ変換するBox2D adapterの失敗テストを作る。
3. 地面、重力、collision、4〜6ボーンfixtureを実装する。
4. `EpisodeRunner` とcleanup契約を実装する。
5. 10,000 stepと100回cleanupのintegration testを通す。
6. M1検証結果をdocsへ追加し、M2へ進めるか判定する。
