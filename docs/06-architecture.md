...
M7時点の実構成です。当初のディレクトリ案から、実装に合わせて名前と粒度を調整しています。
...
**状態: 実装済み（M6）。** `src/ui/app-state.ts` が純粋な状態機械として持ちます。DOM・Canvas・物理を知らないため、ブラウザを起動せずに「学習中は形を差し替えられない」「速度を変えても評価の定義は変わらない」を試験できます（[docs/19](19-m6-experience-review.md) §5）。開発ページ `bench/stroke-input.html` は状態機械を持たないまま残しています。
...
最初からWorker化はしません。実測（Population 32 で実時間の37倍、観察フェーズは最大構成でも1 stepあたり0.29 ms）からは、計算量そのものには余裕があります。

ただし**学習フェーズはメインスレッドを止めます**。1世代の評価は骨20本で約0.47秒かかり、製品画面は1フレームに1世代ずつ進めることで進捗を出しています（[D-011](09-risks-open-questions-and-decisions.md)）。世代内をさらに分けるか、Worker化するかは、ブラウザ描画を含めた計測後に再判断します。
