...
| ID | リスク | 影響 | 現在の対策 | 状態 |
|---|---|---|---|---|
| R-01 | 自由描線が短すぎる骨を大量生成 | 物理不安定、遅延 | resampling、位相分割、長さ・本数上限 | **M4で対策実装・fixtureで確認済み** |
| R-02 | RDPが戻り線を消す | 枝が失われる | 位相イベント検出後にedge単位で簡略化 | **M4で区間内RDPを実装・戻り線自体はM5** |
| R-03 | 閉ループが過拘束になる | 発散、震え | P4まで保留、substepと質量比を実測 | **未確認** |
| R-04 | 多個体でブラウザが重い | 学習体験悪化 | 表示と計算を分離、32体から測定 | **headlessで余裕を確認（実時間37倍。観察は最大構成でも1 stepあたり0.29 ms）・ブラウザ未確認** |
...
### D-011: 長い線は拒否せず、骨を粗くして受け入れる

- 状態: **決定済み（M6、ユーザー要望 2026-09-14）**
- 決定: 骨数が上限を超えたら、骨の上限長と簡略化の許容誤差を粗くして組み直す。あわせて `maxEdgeCount` を 14 → **20**、`DEFAULT_GRAPH_LIMITS` を `maxEdgeCount` 24・`maxTotalLength` 40 m へ。粗くできる骨長は 2 m まで。
- 理由: 「骨が20本になり、上限の14本を超えます」で入り組んだ絵が描けませんでした。長さを理由に拒否するより、骨を粗くして受け入れる方が体験として自然です。
- 影響: 描線の総延長の98〜99%が保たれます。最小骨長は粗くしません（上げると形が潰れるため）。骨20本では Population 32・20世代の学習が約9.5秒かかるため、**1世代ずつフレームに分けて進める** `EvolutionRunner` を追加しました。
- 再評価: M7の長時間runと性能測定。骨20本での p95 frame time は未測定です。
