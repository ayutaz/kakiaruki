...
| ブラウザE2E | Playwright系 | **保留（ユーザー判断）** | Pointer入力、UI、長時間実行、描画有無の結線を確認できる。M4時点で未導入。判断材料は [M4検証結果](17-m4-stroke-input-validation.md) §8 |
...
| 3 | 32個体の固定骨格を、対象PCの前景タブで少なくとも実時間相当以上に進められない | **headlessでは解消済み**（実時間の44倍）。**前景タブでの実測は未実施** |
