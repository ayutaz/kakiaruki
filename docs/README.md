...
**カキアルキ**（`ayutaz/kakiaruki`）は、公開されているゲーム『一筆進化』の体験を参考に、Web向けの独立した再現実装が技術的に成立するかを検証するためのプロジェクトです。ライセンスは Apache-2.0 です。製品画面は <https://ayutaz.github.io/kakiaruki/> で公開しています（2026-09-14〜）。
...
- **確認済み**: 1 World内でPopulation 32を実時間の37倍の速さで評価でき、個体間contactは0件、100世代でshape数が基準へ戻る（headless計測）。
- **確認済み**: 5 Seed×50世代で進化群が5/5改善し、進化なし対照群の中央値（4.17）を大きく上回った（17.46）。移動距離は1.2〜2.2体長から5.0〜6.5体長へ。
- **確認済み**: 30分の連続運転で273周・5,460世代・約174,700個体評価。未処理例外0件、heapは前半88.3 MB → 後半88.8 MB（1.01倍）。
...
- 製品画面でPhaserを使うか、Canvas 2Dで足りるか（[M6検証結果](19-m6-experience-review.md) §9）。現在はCanvas 2Dで実装し、build出力が 1,541 kB → 217 kB になりました。
