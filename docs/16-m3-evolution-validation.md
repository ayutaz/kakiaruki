...
M3の受入条件（§9）のうち、**自動試験と実験で検証できる7項目すべてに合格**しました。固定Seedの5試行すべてで進化群が generation 0 を上回り、進化なし対照群の中央値も明確に上回りました。
...
地面の半幅を 200 m → **1000 m** にしました。Population 32 で最大骨格（1.2 m × 14本）を並べても端は ±446 m で、そこから `maxDisplacement` 200 m 進んでも地面が続きます。
