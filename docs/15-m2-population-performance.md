...
M2の受入条件（§6）のうち、**自動試験で検証できる7項目すべてに合格**しました。1つのWorld内に互いに干渉しないレーンを作り、Population 1／8／32を完走させ、100世代相当の生成・cleanupで資源が基準へ戻ることを確認しています。
...
## 12. その後の修正と再測定（2026-09-14）

§5 の実時間比は**地面の幅が足りない状態**で測ったものです。`zigzag6` のPopulation 32では、外側の6個体が地面の外に生成され、6秒間ずっと落下していました（[M4検証結果](17-m4-stroke-input-validation.md) §11「原因4」）。落下中の個体は接触計算をほとんど行わないため、**性能を実際より良く見せていました**。

地面の半幅を 200 m → 1000 m に直した後の再測定:

```text
npm run bench
  pop | world steps | episode wall s | physics steps/s | creature steps/s | real-time ratio
    1 |         360 |         0.0074 |           48543 |            48543 |           809.1
    8 |         360 |         0.0408 |            8834 |            70669 |           147.2
   32 |         360 |         0.1547 |            2327 |            74463 |            38.8
  Population 32 real-time ratio = 38.8x (threshold 1.0x): PASS
```

| 項目 | 修正前 | 修正後 |
|---|---:|---:|
| Population 32 実時間比 | 44.1x | **38.8x** |
| Population 8 実時間比 | 147.4x | 147.2x |
| 実際に地面の上にいた個体 | 26 / 32 | 32 / 32 |

**受入条件（Population 32で実時間比1.0以上）の判定は変わりません。** 閾値は測定前に固定したものから変更していません。低下分は、これまで評価できていなかった6個体ぶんの実コストです。
