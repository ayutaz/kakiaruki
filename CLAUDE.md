...
現在地点は **M7（安定化と公開判断）まで技術検証済み**。描く → `CreatureGraph` へ変換（なぞって戻ると枝分かれ）→ 1 World内のレーンで複数個体を評価 → GAで世代交代 → 世代を並べて観察 → 描き直し、までを **`index.html`（製品画面）だけで完走できます**。実装のマイルストーンは終わり、残るのは人の確認と決定（体験品質・ブラウザ確認・名称・ライセンス・公開判断）です。
...
npm run soak        # 長時間run。既定30分。npm run soak -- 5 で5分
```
